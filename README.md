...
The traffic and roadside equipment tile and Fleet band use a drawn traffic
control plan on a rural major road (`road-plan.svg`, generated by
`road-plan.py` and rendered to `road-plan.png`), labelled illustrative only. No tile on the site ships without an image, and nothing is
